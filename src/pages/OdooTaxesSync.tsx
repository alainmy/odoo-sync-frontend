...
import { Loader2, RefreshCw, Search, Filter, Percent } from 'lucide-react';
...
export default function OdooTaxesSync() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery<TaxListResponse>({
    queryKey: ['odoo-taxes-sync', search, filterStatus, page],
    queryFn: async () => {
      ...
      try {
        const response = await api.get(`/api/v1/sync-management/taxes?${params}`);
        ...
        toast({
          ...
          description: error.response?.data?.detail || 'Failed to fetch taxes',
        });
        throw error;
      }
    },
  ...
  const { data: stats } = useQuery<SyncStatsResponse>({
    queryKey: ['tax-sync-stats'],
    queryFn: async () => {
      const response = await api.get('/api/v1/sync-management/taxes/statistics');
      return response.data;
    },
    refetchInterval: 30000,
  });
  ...
  const batchSyncMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await api.post('/api/v1/sync-management/taxes/batch-sync', {
        odoo_ids: ids,
    ...
    onSuccess: (data) => {
      toast({
        title: 'Sync Started',
        description: `${data.total_taxes} taxes queued for synchronization`,
      });
      setSelectedIds([]);
      setTimeout(() => {
        queryClient.invalidateQueries({ queryKey: ['odoo-taxes-sync'] });
        queryClient.invalidateQueries({ queryKey: ['tax-sync-stats'] });
      }, 2000);
    },
    onError: (error: any) => {
      toast({
  ...
  const toggleSelectAll = () => {
    ...
    if (selectedIds.length === data.taxes.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(data.taxes.map(t => t.odoo_id));
    }
  };
  ...
  const handleSync = () => {
    if (selectedIds.length === 0) {
      toast({
        ...
        description: 'Please select at least one tax to sync',
      });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Odoo Taxes Sync</h1>
          <p className="text-muted-foreground">
            Manage synchronization of taxes from Odoo to WooCommerce
          </p>
        </div>
        <Button
          onClick={() => refetch()}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Total: {totalCount} taxes | Selected: {selectedIds.length}
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Price Include</TableHead>
                  ...
                  <TableBody>
                    {taxes.length === 0 ? (
                      ...
                      taxes.map((tax) => (
                        <TableRow key={tax.odoo_id}>
                          <TableCell>
                            <Checkbox
                              checked={selectedIds.includes(tax.odoo_id)}
                              onCheckedChange={() => toggleSelect(tax.odoo_id)}
                            />
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <Badge
                              variant={statusVariants[tax.sync_status].variant}
                              className={statusVariants[tax.sync_status].color}
                            >
                              {statusVariants[tax.sync_status].label}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            {tax.woocommerce_id || '-'}
